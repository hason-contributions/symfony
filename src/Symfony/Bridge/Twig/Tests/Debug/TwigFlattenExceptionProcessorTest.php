...
class TwigFlattenExceptionProcessorTest extends \PHPUnit_Framework_TestCase
{
    public function testProcess()
    {
        ...
        $this->assertEquals(array(array('line' => 4, 'file' => $files['layout'])), $trace->get(-1, 'related_codes'));
        $this->assertEquals(array(array('line' => 4, 'file' => $files['layout'])), $trace->get(0, 'related_codes'));
        $this->assertEquals(array(array('line' => 3, 'file' => $files['index'])), $trace->get(3, 'related_codes'));
        $this->assertEquals(array(array('line' => 1, 'file' => $files['base'])), $trace->get(5, 'related_codes'));
        $this->assertEquals(array(array('line' => 1, 'file' => $files['layout'])), $trace->get(8, 'related_codes'));
        $this->assertEquals(array(array('line' => 1, 'file' => $files['index'])), $trace->get(11, 'related_codes'));
    }
    ...
    public function testProcessRealTwigFile()
    {
        $file = (object) array(
            ...
            'path' => __DIR__.'/../Fixtures/templates/error.html.twig',
            'type' => 'twig'
        );
        ...
        $this->assertEquals(array(array('line' => 2, 'file' => $file)), $trace->get(-1, 'related_codes'));
    }
}
