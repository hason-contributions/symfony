...
class PatternParser
{
     ...
     */
    public function parse($pattern, $apostropheMode = self::APOSTROPHE_DOUBLE_OPTIONAL)
    {
        $this->pattern = str_replace(array("\r\n", "\r"), "\n", $pattern);
        $this->cursor = 0;
        $this->lineno = 1;
        $this->end = strlen($this->pattern);

        preg_match_all("/(['\{\}])/", $this->pattern, $matches, PREG_OFFSET_CAPTURE);
